...
export const getName = async (cookie) => {
  try {
    ...
    return await makeScrap(options);
  } catch (error) {
    if (error.statusCode === 303) {
      throw new Error('Não logado');
    }
  ...
  try {
    ...
    const optionsProfile: MakeScrap = {
      ...
      scrapper: async ($) => {
        ...
        return await makeScrap(optionsDetails);
      },
    };
    return await makeScrap(optionsProfile);
  } catch (error) {
    if (error.statusCode === 303) {
      throw new Error('Não logado');
    }
...
export const getAcademicCalendar = async (cookie) => {
  try {
    const optionsAcademic: MakeScrap = {
      ...
      scrapper: async ($$) => {
        ...
        const embedpage = $$('[name="Embpage1"]').attr('src');
        if (!embedpage) {
          throw new Error('Não logado');
        }

        const academicUrl = getAcademicUrl(embedpage);
    ...
    return await makeScrap(optionsAcademic);
...
export const getSchoolGrade = async (cookie) => {
  try {
...
export const getHistory = async (cookie) => {
...
export const getSchedules = async (cookie) => {
  try {
    const options: MakeScrap = {
      cookie,
      url: ROUTES.SCHEDULE,
...
export const getRegisteredEmails = async (cookie) => {
  try {
    const options: MakeScrap = {
      cookie,
      url: ROUTES.HOME,
...
export const getPartialGrades = async (cookie) => {
  try {
...
export const getEnrolledDisciplines = async (cookie) => {
  try {
